...
<head>
    ...
    <title>REPORTE DETALLADO POR PLANES DE MEDIOS</title>
    <style>
...
<body>
    ...
    <header>
        <div class="header_adp">
            <div class="title">
                <p style="color: #FF3333;"><strong>REPORTE DETALLADO POR PLANES DE MEDIOS</strong></p>
            </div>
            <div class="logo">
    ...
    @if (!$lastPage) 
        ...
        <section class="first-page">
            <div class="container-table">
                ...
                <p><strong>Publicaciones por planes de medios:</strong></p>
            </div>
            @if ($isVal || $isDet)
            <div class="container-table">
            ...
            <div class="container-table">
                <table class="statistics-table">
                    <thead>
                        <tr>
                            <th style="text-align: center;">Planes de Medios</th>
                    ...
                    <tbody>
                    @foreach($planMediosTotales as $planMedioTotal)
                        <tr>
                            <td style="text-align: left;">{{$planMedioTotal->nombre}}</td>
                            @foreach($plataformas as $plataforma)
                            <td style="text-align: center;">{{($count[$planMedioTotal->id][$plataforma->id] > 0) ? $count[$planMedioTotal->id][$plataforma->id] : '-'}}</td>
                            @endforeach
                            <td style="text-align: center; font-weight: bold;">{{($count[$planMedioTotal->id]['total'] > 0) ? $count[$planMedioTotal->id]['total'] : '-'}}</td>
                        </tr>
                    @endforeach
                        <tr>
                            <td style="text-align: center; font-weight: bold;">TOTAL</td>
                            @foreach($plataformas as $plataforma)
                            <td style="text-align: center; font-weight: bold;">{{($count['total'][$plataforma->id] > 0) ? $count['total'][$plataforma->id] : '-'}}</td>
                            @endforeach
        ...
        <section class="second-page">
            <div class="container-table">
            @foreach($planMedios as $planMedio)
                @if ($printPageBreak)
                ...
                <table>
                    <thead>
                        <tr>
                            <th style="text-align: left; text-transform: uppercase; font-weight: bold;" colspan="2">{{$planMedio->nombre}}</th>
                        </tr>
                    </thead>
                    <tbody>
                    @foreach($plataformas as $plataforma)
                    @if ($count[$planMedio->id][$plataforma->id] > 0)
                        @foreach($data as $item)
                        @if ($planMedio->id === $item->idPlanMedio  && $plataforma->id === $item->idPlataforma)
                        <tr>
                            <td style="width: 50%;">
    ...
    @if ($lastPage) 
        <section class="third-page">
            ...
            @else
                <img src="../storage/app/clientes/logo.png" width="100" alt="">
            @endif
                <p>REPORTE DETALLADO POR PLANES DE MEDIOS</p>
            </div>
        </section>
    @endif    
    </main>
