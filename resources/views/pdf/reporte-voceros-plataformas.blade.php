...
<head>
    ...
    <title>REPORTE DETALLADO POR VOCEROS</title>
    <style>
...
<body>
    ...
    <header>
        <div class="header_adp">
            <div class="title">
                <p style="color: #FF3333;"><strong>REPORTE DETALLADO POR VOCEROS</strong></p>
            </div>
            <div class="logo">
    ...
    @if (!$lastPage) 
        ...
        <section class="first-page">
            <div class="container-table">
                ...
                <p><strong>Publicaciones por voceros:</strong></p>
            </div>
            @if ($isVal || $isDet)
            <div class="container-table">
            ...
            <div class="container-table">
                <table class="statistics-table">
                    <thead>
                        <tr>
                            <th style="text-align: center;">Voceros</th>
                    ...
                    <tbody>
                    @foreach($vocerosTotales as $voceroTotal)
                        <tr>
                            <td style="text-align: left;">{{$voceroTotal->nombres.' '.$voceroTotal->apellidos}}</td>
                            @foreach($plataformas as $plataforma)
                            <td style="text-align: center;">{{($count[$voceroTotal->id][$plataforma->id] > 0) ? $count[$voceroTotal->id][$plataforma->id] : '-'}}</td>
                            @endforeach
                            <td style="text-align: center; font-weight: bold;">{{($count[$voceroTotal->id]['total'] > 0) ? $count[$voceroTotal->id]['total'] : '-'}}</td>
                        </tr>
                    @endforeach
                        <tr>
        ...
        <section class="second-page">
            ...
            @foreach($voceros as $vocero)
                ...
                <table>
                    <thead>
                        <tr>
                            <th style="text-align: left; text-transform: uppercase; font-weight: bold;" colspan="2">{{$vocero->nombres.' '.$vocero->apellidos}}</th>
                        </tr>
                    </thead>
                    <tbody>
                    @foreach($plataformas as $plataforma)
                    @if ($count[$vocero->id][$plataforma->id] > 0)
                        @foreach($data as $item)
                        @if ($vocero->id === $item->idVocero  && $plataforma->id === $item->idPlataforma)
                        <tr>
                            <td style="width: 50%;">
    ...
    @if ($lastPage) 
        <section class="third-page">
            ...
            @else
                <img src="../storage/app/clientes/logo.png" width="100" alt="">
            @endif
                <p>REPORTE DETALLADO POR VOCEROS</p>
            </div>
        </section>
    @endif    
    </main>
