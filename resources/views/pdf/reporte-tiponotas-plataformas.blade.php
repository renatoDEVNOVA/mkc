...
<head>
    ...
    <title>REPORTE DETALLADO POR TIPOS DE NOTA</title>
    <style>
...
<body>
    ...
    <header>
        <div class="header_adp">
            <div class="title">
                <p style="color: #FF3333;"><strong>REPORTE DETALLADO POR TIPOS DE NOTA</strong></p>
            </div>
            <div class="logo">
    ...
    @if (!$lastPage) 
        ...
        <section class="first-page">
            <div class="container-table">
                ...
                <p><strong>Publicaciones por tipos de nota:</strong></p>
            </div>
            @if ($isVal || $isDet)
            <div class="container-table">
            ...
            <div class="container-table">
                <table class="statistics-table">
                    <thead>
                        <tr>
                            <th style="text-align: center;">Tipos de Nota</th>
                            @foreach($plataformasTotales as $plataformaTotal)
                            <th style="text-align: center;">{{$plataformaTotal->descripcion}}</th>
                            @endforeach
                            <th style="text-align: center;">TOTAL</th>
                        </tr>
                    </thead>
                    <tbody>
                    @foreach($tipoNotasTotales as $tipoNotaTotal)
                        <tr>
                            @switch($tipoNotaTotal)
                                @case(1)
                                    <td style="text-align: center;">Nota</td>
                                    @break
                                @case(2)
                                    <td style="text-align: center;">Entrevista</td>
                                    @break
                                @case(3)
                                    <td style="text-align: center;">Reseña</td>
                                    @break
                                @case(4)
                                    <td style="text-align: center;">Opinión</td>
                                    @break
                                @default
                            ...
                            @endswitch
                            @foreach($plataformasTotales as $plataformaTotal)
                            <td style="text-align: center;">{{($count[$tipoNotaTotal][$plataformaTotal->id] > 0) ? $count[$tipoNotaTotal][$plataformaTotal->id] : '-'}}</td>
                            @endforeach
                            <td style="text-align: center; font-weight: bold;">{{($count[$tipoNotaTotal]['total'] > 0) ? $count[$tipoNotaTotal]['total'] : '-'}}</td>
                        </tr>
                    @endforeach
                        <tr>
                            <td style="text-align: center; font-weight: bold;">TOTAL</td>
                            @foreach($plataformasTotales as $plataformaTotal)
                            <td style="text-align: center; font-weight: bold;">{{($count['total'][$plataformaTotal->id] > 0) ? $count['total'][$plataformaTotal->id] : '-'}}</td>
                            @endforeach
                ...
                <table>
                    <thead>
                        <tr>
                            @switch($tipoNota)
                                @case(1)
                                    <th style="text-align: left; text-transform: uppercase; font-weight: bold;" colspan="2">Nota</th>
                                    @break
                                @case(2)
                                    <th style="text-align: left; text-transform: uppercase; font-weight: bold;" colspan="2">Entrevista</th>
                                    @break
                                @case(3)
                                    <th style="text-align: left; text-transform: uppercase; font-weight: bold;" colspan="2">Reseña</th>
                                    @break
                                @case(4)
                                    <th style="text-align: left; text-transform: uppercase; font-weight: bold;" colspan="2">Opinión</th>
                                    @break
                                @default
                                    <th style="text-align: left; text-transform: uppercase; font-weight: bold;" colspan="2">-</th>
                                    @break
                            @endswitch
                        </tr>
                    </thead>
                    <tbody>
                    @foreach($dataTipoNotaPlat as $item)
                        @if ($plataforma->id === $item->idPlataforma)
                        <tr>
                            <td style="width: 50%;">
                                <ul style="list-style: none;">
                                <li>
                                @switch($item->idPlataforma)
                                    @case(1)
                                        <div style="color: #FFFFFF; text-transform: uppercase; font-size: 10px; font-weight: bold; background-color: #B0E5ED; margin: 10px 0px; height: 20px; line-height: 10px; width: 80px; text-align: center; border-radius: 2px;">{{$plataforma->descripcion}}</div>
                                        @break
                                    @case(2)
                                        <div style="color: #FFFFFF; text-transform: uppercase; font-size: 10px; font-weight: bold; background-color: #02704B; margin: 10px 0px; height: 20px; line-height: 10px; width: 80px; text-align: center; border-radius: 2px;">{{$plataforma->descripcion}}</div>
                                        @break
                                    @case(3)
                                        <div style="color: #FFFFFF; text-transform: uppercase; font-size: 10px; font-weight: bold; background-color: #FE6D34; margin: 10px 0px; height: 20px; line-height: 10px; width: 80px; text-align: center; border-radius: 2px;">{{$plataforma->descripcion}}</div>
                                        @break
                                    @case(5)
                                        <div style="color: #FFFFFF; text-transform: uppercase; font-size: 10px; font-weight: bold; background-color: #1A1AB5; margin: 10px 0px; height: 20px; line-height: 10px; width: 80px; text-align: center; border-radius: 2px;">{{$plataforma->descripcion}}</div>
                                        @break
                                    @case(9)
                                        <div style="color: #FFFFFF; text-transform: uppercase; font-size: 10px; font-weight: bold; background-color: #FFD731; margin: 10px 0px; height: 20px; line-height: 10px; width: 80px; text-align: center; border-radius: 2px;">{{$plataforma->descripcion}}</div>
                                        @break
                                    @default
                                        <div style="color: #FFFFFF; text-transform: uppercase; font-size: 10px; font-weight: bold; background-color: #FF3333; margin: 10px 0px; height: 20px; line-height: 10px; width: 80px; text-align: center; border-radius: 2px;">{{$plataforma->descripcion}}</div>
                                        @break
                                @endswitch
                                </li>
    ...
    @if ($lastPage) 
        <section class="third-page">
            ...
            @else
                <img src="../storage/app/clientes/logo.png" width="100" alt="">
            @endif
                <p>REPORTE DETALLADO POR TIPOS DE NOTA</p>
            </div>
        </section>
    @endif    
    </main>
