...
use Mail;
use App\Mail\ReporteAutomatico as ReporteAutomaticoMail;
...
class GenerarReporte extends Command
{
     ...
     * @var string
     */
    protected $signature = 'generate:reporte';
     ...
     * @var string
     */
    protected $description = 'Genera los reportes';
    ...
    public function handle()
    {
        ...
        $clienteEnvios = ClienteEnvio::all();
        
        foreach ($clienteEnvios as $clienteEnvio) {
            # code...
            $cliente = Cliente::find($clienteEnvio->cliente_id);
            Log::info('Cliente: '.$cliente->nombreComercial);

            $cron = new CronExpression($clienteEnvio->cron);
            $isDue = $cron->isDue($time);
            Log::info('CRON: '.$clienteEnvio->cron);
            Log::info('isDue: '.$isDue);

            $destinatarios = $cliente->users()->where('sendAuto', 1)->get();
            Log::info('Contador Destinatarios: '.count($destinatarios));

            if($isDue && (count($destinatarios) > 0)){

                switch ($clienteEnvio->tipoFrecuencia) {
                  case 1:
                      # Semanal
                ...
                if (in_array(1, $tiposReporte)) {
                    ...
                    if(count($data) > 0){
                        ...
                        $reporte = Reporte::create([
                            ...
                            'tipoFrecuencia' => $clienteEnvio->tipoFrecuencia,
                            //'numPeriodo' => $M,
                            //'year' => $Y,
                            'tipoReporte' => 1,
                        ]);
                ...
                Log::info('Fin Generar Reporte');


                Log::info('Inicio Envio Reporte');
                  ...
                  if ($validatorEmail->fails()) {
                      ...
                      if (in_array(1, $tiposReporte)) {
                          ...
                          if(!$emptyData[1]){
                              ...
                              Log::info('Reporte Plataformas');

                              $tituloCorreo = "REPORTE POR PLATAFORMAS";
                              $asunto = strtoupper($cliente->nombreComercial).": REPORTE POR PLATAFORMAS (del ".date('d-m-Y', strtotime($fechaInicio))." al ".date('d-m-Y', strtotime($fechaFin)).")";
                              $fileName = $filesName[1];

                              try {

                                  Mail::to($destinatario->email, $destinatario->name)
                                  ->send(new ReporteAutomaticoMail($tituloCorreo, $destinatario, $fileName, $fechaInicio, $fechaFin, $asunto));
                      ...
                      if (in_array(2, $tiposReporte)) {
                          ...
                          if(!$emptyData[2]){
                              ...
                              Log::info('Reporte Campañas');

                              $tituloCorreo = "REPORTE POR CAMPAÑAS";
                              $asunto = strtoupper($cliente->nombreComercial).": REPORTE POR CAMPAÑAS (del ".date('d-m-Y', strtotime($fechaInicio))." al ".date('d-m-Y', strtotime($fechaFin)).")";
                      ...
                      if (in_array(3, $tiposReporte)) {
                              ...
                              Log::info('Reporte Voceros');

                              $tituloCorreo = "REPORTE POR VOCEROS";
                              $asunto = strtoupper($cliente->nombreComercial).": REPORTE POR VOCEROS (del ".date('d-m-Y', strtotime($fechaInicio))." al ".date('d-m-Y', strtotime($fechaFin)).")";
