...
use Mail;
use App\Mail\ReporteSemanal as ReporteSemanalMail;
...
class GenerarReporteSemanal extends Command
{
     ...
     * @var string
     */
    protected $signature = 'generate:reporte_semanal';
     ...
     * @var string
     */
    protected $description = 'Genera los reportes semanales';
    ...
    public function handle()
    {
        ...
        foreach ($clientes as $cliente) {
            # code...
            $clienteEnvio = $cliente->envios()->where('tipoPeriodo', 1)->first();
            ...
            if(!empty($clienteEnvio) && (count($destinatarios) > 0)){
                ...
                if($clienteEnvio->diaEnvio == $dayCurrent && $clienteEnvio->horaEnvio == $hourCurrent){
                    Log::info('Inicio GenerateWeekly');
                    ...
                    Log::info('Fin GenerateWeekly');


                    Log::info('Inicio SendWeekly');
                        ...
                        if ($validatorEmail->fails()) {
                            ...
                            if (in_array(1, $tiposReporte)) {
                                ...
                                if(!$emptyData[1]){
                                    ...
                                    $tituloCorreo = "REPORTE SEMANAL POR PLATAFORMAS";
                                    $asunto = strtoupper($cliente->nombreComercial).": REPORTE SEMANAL POR PLATAFORMAS (del ".date('d-m-Y', strtotime($fechaInicio))." al ".date('d-m-Y', strtotime($fechaFin)).")";
                                    $fileName = $filesName[1];

                                    try {

                                        Mail::to($destinatario->email, $destinatario->name)
                                        ->send(new ReporteSemanalMail($tituloCorreo, $destinatario, $fileName, $fechaInicio, $fechaFin, $asunto));
                            ...
                            if (in_array(2, $tiposReporte)) {
                                ...
                                if(!$emptyData[2]){
                                    ...
                                    $tituloCorreo = "REPORTE SEMANAL POR CAMPAÑAS";
                                    $asunto = strtoupper($cliente->nombreComercial).": REPORTE SEMANAL POR CAMPAÑAS (del ".date('d-m-Y', strtotime($fechaInicio))." al ".date('d-m-Y', strtotime($fechaFin)).")";
                            ...
                            if (in_array(3, $tiposReporte)) {
                                    ...
                                    $tituloCorreo = "REPORTE SEMANAL POR VOCEROS";
                                    $asunto = strtoupper($cliente->nombreComercial).": REPORTE SEMANAL POR VOCEROS (del ".date('d-m-Y', strtotime($fechaInicio))." al ".date('d-m-Y', strtotime($fechaFin)).")";
